...
class startup:
    """initial frame if GradeBook is run with no commands"""
    def __init__(self, root, font="Helvetica", fontsize=12):
        ...
        self.version_label = tk.Label(self.master, text="version {0}".format(m.version), font=(font,fontsize))
        self.version_label.grid(row=1,column=1)

        self.create_button = tk.Button(self.master, text="Create", font=(font, fontsize), command=self.new, state=tk.Active)
